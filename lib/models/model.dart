...
class Model extends ChangeNotifier {
  ...
  Future<String> createOrder({
    ...
    final tax = subtotal * 0.21; // 10% tax
    final shipping = subtotal > 50 ? 0 : 10; // Free shipping over $50
  ...
  // Add these helper methods to your Model class in model.dart

  /// Checks if an appointment can be marked as completed
  /// Rules:
  /// - Must be on or after the appointment date
  /// - Must be at least 15 minutes into the appointment time
  ...
  bool canCompleteAppointment(Appointment appointment) {
    ...
    // Must be at least 15 minutes into the appointment
    final completionEligibleTime = appointmentDateTime.add(const Duration(minutes: 15));

    return now.isAfter(completionEligibleTime);
  }

  /// Checks if an appointment can be marked as confirmed
  /// Rules:
  /// - Must be pending status
  /// - Cannot confirm past appointments (they should be completed or no-show)
  bool canConfirmAppointment(Appointment appointment) {
    ...
    // Can confirm if appointment is in the future or happening today
  ...
  /// - Cannot cancel past appointments (mark as no-show instead)
  ...
  /// - Typically for appointments that were confirmed but client didn't show
  ...
  /// Gets the status text with additional no-show status
  ...
  /// Helper method to parse appointment date and time into a single DateTime
  DateTime parseAppointmentDateTime(Appointment appointment) {
    try {
      ...
      if (timeParts.length != 2) {
        // Fallback to appointment date if time parsing fails
  ...
  /// Gets available status transitions for an appointment
  ...
  /// Gets a human-readable reason why a status change is not allowed
